    ...
    <aside className="hidden lg:flex flex-none w-80 flex-col gap-4 z-20">
      <div className="border border-primary/30 bg-background-dark/80 backdrop-blur-sm p-4 h-full flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-4 border-b border-primary/20 pb-2">
          <span className="text-[10px] text-primary font-mono font-bold tracking-[0.2em] uppercase">
            {aboutTranslations.sidebarLeft?.networkStatus || "NETWORK_STATUS_FEED"}
          </span>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 font-mono text-[10px] pr-2 custom-scrollbar">
          {feedItems.map((item, idx) => (
            ...
            >
              <p className="text-primary/40">[{item.time}] {item.action}</p>
        ...
        <div className="mt-4 pt-2 border-t border-primary/20">
          <div className="flex justify-between items-center text-[9px] text-primary/60 font-mono">
            <span>{aboutTranslations.sidebarLeft?.bandwidthUsage || "BANDWIDTH_USAGE"}</span>
            <span>84.2 GB/s</span>
          </div>
